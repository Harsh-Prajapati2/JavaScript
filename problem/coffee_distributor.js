...
"items" - an array of objects representing the items ordered, where each item has a 
"name" and a "price" property
"discountCode" - a string representing a discount code for the order (optional)

Requirement 2:

You need to write a function "totalOrderValue" that takes an array of order objects 
and a callback function as its input and returns the total value of all orders combined 
after applying any applicable discounts.
The callback function, named "applyDiscount" should take two parameters: a discount 
code and total order value for a particular object, and applies the discount based on the following condition.
...
function totalOrderValue(orders, applyDiscount) {
  return orders
    .map(order => {
      ...
      // Apply discount if discountCode is present
...
const applyDiscount = (discountCode, total) => {
  if (discountCode === "COFFEELOVER") {
    ...
    return total * 0.80; // Apply 20% discount
  }
...
console.log(result); // Output: 12.08
