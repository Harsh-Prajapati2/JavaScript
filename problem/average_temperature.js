...
function calculateAverage(pressureThreshold) {
  // Filter cities with pressure equal to or above the threshold
  const filteredCities = weather.filter(city => city.pressure >= pressureThreshold);
  
  // Map temperatures from the filtered cities
  const temperatures = filteredCities.map(city => city.temperature);
  
  // Reduce to calculate the total sum of temperatures
  const totalTemperature = temperatures.reduce((sum, temp) => sum + temp, 0);
  
  // Calculate the number of cities that meet the pressure condition
  const numberOfCities = filteredCities.length;
  
  // Calculate the average temperature and round to one decimal place
  const averageTemperature = (totalTemperature / numberOfCities).toFixed(1);
  
  return parseFloat(averageTemperature);
}

console.log(calculateAverage(1015)); // Output: 68.3
